...
const mixArr: PrimitiveTypes[] = [42, 7, 19, 100, 3, "apple", "banana", "cherry", "grape", "mango", "hello", 25, true, "world", false,];
...
let numArr: PrimitiveTypes[] = [2, 17, 13, 6, 22, 31, 45, 66, 100, -18];
...
for (let i: number = 0; i < numArr.length; i++) {
    let item: PrimitiveTypes = numArr[i];
...
for (let i: number = numArr.length - 1; i >= 0; i--) {
    console.log(numArr[i]);
}
...
for (let i: number = numArr.length; i >= 0; i--) {
    let item: PrimitiveTypes = numArr[i];
    if (typeof item === 'number' && item % 3 === 0) {
        numArr[i] = 'okten';
    }
...
const strings: string[] = ["apple", "banana", "cherry", "grape", "mango", "orange", "peach", "plum", "kiwi", "melon"];
...
type MixedTypes = PrimitiveTypes | null | undefined | number[] | { id: number, key: string };

const mixedArr: MixedTypes[] = [42, 'Hello, Okten', true, null, undefined, {
    id: 1,
    key: 'value'
}, [1, 2, 3], false, NaN, 99,];
...
const threeTypesArr: PrimitiveTypes[] = [42, "apple", true, 7, "banana", false, 99, "cherry", true, 18];
...
for (let i = 0; i < 10; i++) {
    if (typeof arrForFill[i] === 'number') {
        if (i % 3 === 0) {
            arrForFill[i] = `Element with index ${i}`;
        } else if (i % 3 === 1) {
            arrForFill[i] = String(i * 512);
        } else {
            arrForFill[i] = String(i % 2 === 0);
        }
    }
}
...
let maxAuthor: number = 0;

for (const book of books) {
    const numOfAuthors: number = book.authors.length;
...
let maxLengthTitle: number = 0;

for (const book of books) {
    const lengthTitle: number = book.title.length;
